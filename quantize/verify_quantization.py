...
def level_2_layerwise_activation_verification(fp32_model_path="best.onnx", 
    ...
    def get_intermediate_outputs(model_path, node_indices):
        m = onnx.load(model_path)
        tensor_names = [m.graph.node[idx].output[0] for idx in node_indices]
        ...
        outs = sess.run(tensor_names, {input_name: inp})
        return {idx: outs[i] for i, idx in enumerate(node_indices)}

    print("[INFO] Đang chạy suy luận FP32 và INT8 trên toàn bộ intermediate nodes...")
    outs_fp32 = get_intermediate_outputs(fp32_model_path, check_nodes)
    outs_int8 = get_intermediate_outputs(int8_model_path, check_nodes)
