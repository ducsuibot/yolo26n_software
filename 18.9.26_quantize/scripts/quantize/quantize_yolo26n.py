...
Outputs:
    ...
    3. best_quantized_int8.onnx:
       - True INT8 QLinearConv model: Conv input INT8, weights INT8, bias INT32, Conv output INT8
       - No dequantization before/inside Conv blocks
"""
...
def calibrate_activations(onnx_path, conv_nodes, val_img_dir="DUT_Anti_UAV_YOLO/images/val", ref_img="DUT_Anti_UAV_YOLO/images/val/00068.jpg", max_samples=100):
    """
    ...
    if not calib_images and os.path.exists("val_dataset/images"):
        calib_images = sorted(glob.glob("val_dataset/images/*.jpg"))
    if os.path.exists(ref_img) and ref_img not in calib_images:
        calib_images.insert(0, ref_img)
    if max_samples and max_samples > 0 and len(calib_images) > max_samples:
        calib_images = calib_images[:max_samples]
...
def main():
    ...
    # 4b. Generate Master quantization_params.json for all 207 tensors
    all_tensor_records = []
    for nid, stage, mod, ptype, fpath in all_txt_files:
        vals = []
        ...
        s_sym = float(abs_max / 127.0) if abs_max > 0 else 1.0
        q_sym = np.clip(np.round(arr / s_sym), -128, 127).astype(np.int8)
        dq_sym = q_sym.astype(np.float32) * s_sym
        err_sym = arr - dq_sym
        max_err_sym = float(np.max(np.abs(err_sym))) if len(arr) > 0 else 0.0
        norm_sym = float(np.linalg.norm(arr) * np.linalg.norm(dq_sym))
        cos_sym = float(np.dot(arr, dq_sym) / norm_sym) if norm_sym > 0 else 1.0
        signal_pwr = float(np.sum(arr ** 2))
        noise_pwr = float(np.sum(err_sym ** 2))
        sqnr_sym = float(10.0 * np.log10(signal_pwr / noise_pwr)) if noise_pwr > 0 else 100.0
        
        # Asymmetric
        min_v = float(np.min(arr)) if len(arr) > 0 else 0.0
        max_v = float(np.max(arr)) if len(arr) > 0 else 1.0
        s_asym = float((max_v - min_v) / 255.0) if max_v > min_v else 1.0
        zp_asym = int(np.clip(np.round(-128.0 - min_v / s_asym), -128, 127))
        q_asym = np.clip(np.round(arr / s_asym) + zp_asym, -128, 127).astype(np.int8)
        dq_asym = (q_asym.astype(np.float32) - zp_asym) * s_asym
        err_asym = arr - dq_asym
        max_err_asym = float(np.max(np.abs(err_asym))) if len(arr) > 0 else 0.0
        norm_asym = float(np.linalg.norm(arr) * np.linalg.norm(dq_asym))
        cos_asym = float(np.dot(arr, dq_asym) / norm_asym) if norm_asym > 0 else 1.0
        noise_pwr_asym = float(np.sum(err_asym ** 2))
        sqnr_asym = float(10.0 * np.log10(signal_pwr / noise_pwr_asym)) if noise_pwr_asym > 0 else 100.0
        ...
        all_tensor_records.append({
            ...
            "asymmetric": {
                "scale": s_asym,
                "zero_point": zp_asym,
                "max_err": max_err_asym,
                "cosine_sim": cos_asym,
                "sqnr_db": sqnr_asym
            }
        })
        
    with open("quantization_params.json", 'w', encoding='utf-8') as f:
        json.dump(all_tensor_records, f, indent=2)
    os.makedirs("quantize", exist_ok=True)
    with open(os.path.join("quantize", "quantization_params.json"), 'w', encoding='utf-8') as f:
        json.dump(all_tensor_records, f, indent=2)
    print(f"[SUCCESS] Đã lưu bảng tham số toàn bộ {len(all_tensor_records)} tensors: quantization_params.json")
    ...
    # 6. Build and Export True INT8 QLinearConv ONNX Model (Input INT8, Weight INT8, Bias INT32, Output INT8)
    print("\n[INFO] Đang xây dựng ONNX Model chuẩn QLinearConv (Đầu vào INT8, Trọng số INT8, Bias INT32, Đầu ra INT8)...")
    qlinear_output_path = "best_quantized_int8.onnx"
    ...
    for idx, node in enumerate(model.graph.node):
        ...
        b_orig_name = node.input[2] if len(node.input) > 2 else None
        ...
        # 2. Bias (INT32): S_bias = Sw * Sx
        s_bias = sw * sx
        if b_orig_name and b_orig_name in initializers:
            b_fp32 = initializers[b_orig_name]
            q_b = np.round(b_fp32 / s_bias).astype(np.int32)
            b_int32_name = f"{b_orig_name}_int32"
            new_inits.append(helper.make_tensor(
                b_int32_name, TensorProto.INT32, list(q_b.shape), q_b.flatten().tobytes(), raw=True
            ))
            removed_init_names.add(b_orig_name)
        else:
            q_b = np.zeros((w_fp32.shape[0],), dtype=np.int32)
            b_int32_name = f"{node.name}_bias_int32"
            new_inits.append(helper.make_tensor(
                b_int32_name, TensorProto.INT32, list(q_b.shape), q_b.flatten().tobytes(), raw=True
            ))
            
        # 3. Scales and Zero Points
        scale_x_name = f"sx_{idx}"
        zp_x_name = f"zpx_{idx}"
        scale_w_name = f"sw_{idx}"
        zp_w_name = f"zpw_{idx}"
        scale_y_name = f"sy_{idx}"
        zp_y_name = f"zpy_{idx}"
        ...
        # 4. Input QuantizeLinear: in_tensor -> q_x_out (INT8)
        # Không thực hiện dequantize! Đầu vào trực tiếp của khối Conv là INT8
        q_x_out = f"{in_tensor}_int8_{idx}"
        ...
        new_nodes.append(node_q_x)
        
        # 5. QLinearConv: Nhận đầu vào INT8, Trọng số INT8, Bias INT32 -> Đầu ra INT8
        # Công thức: y = clip(round((Conv(x, w) + B) * (Sx * Sw / Sy)), -128, 127)
        q_y_out = f"{out_orig_name}_int8_{idx}"
        qconv_inputs = [
            q_x_out, scale_x_name, zp_x_name,
            w_int8_name, scale_w_name, zp_w_name,
            scale_y_name, zp_y_name,
            b_int32_name
        ]
        node_qconv = helper.make_node(
            'QLinearConv',
            inputs=qconv_inputs,
            outputs=[q_y_out],
            name=node.name
        ...
        for attr in node.attribute:
            node_qconv.attribute.append(attr)
        new_nodes.append(node_qconv)
        
        # 6. Downstream conversion: DequantizeLinear chỉ để cấp cho các toán tử phi tuyến float (như Sigmoid)
    ...
    q_model = helper.make_model(q_graph, opset_imports=model.opset_import)
    q_model.ir_version = model.ir_version

    onnx.checker.check_model(q_model)
    onnx.save(q_model, qlinear_output_path)
    onnx.save(q_model, "best_quantized_qdq.onnx")
    
    # Đồng bộ lưu cả trong thư mục quantize/
    os.makedirs("quantize", exist_ok=True)
    onnx.save(q_model, os.path.join("quantize", "best_quantized_int8.onnx"))
    onnx.save(q_model, os.path.join("quantize", "best_quantized_qdq.onnx"))
    
    print(f"[SUCCESS] Đã lưu mô hình chuẩn QLinearConv: {qlinear_output_path}")
    print(f"          - Toàn bộ 102 tầng Conv chuyển thành toán tử QLinearConv chuẩn INT8")
    print(f"          - Đầu vào Conv: INT8 | Trọng số Conv: INT8 | Bias: INT32 | Đầu ra Conv: INT8")
    print(f"          - Không thực hiện Dequantize trước khối Conv")
    print(f"          - Dung lượng file: {os.path.getsize(qlinear_output_path) / (1024*1024):.2f} MB")
